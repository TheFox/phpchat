...
class Console extends Thread
{
    ...
    private function handleCommandAddressbook($line)
    {
        ...
        if ($pos === false) {
            ...
            if ($action == 'rem') {
                if ($this->ipcKernelConnection->execSync('addressbookContactRemove', [$id])) {
                        ...
                        if (!$this->getExit()) {
                            ...
                            if ($answer == 'y') {
                                ...
                                try {
                                    ...
                                    if ($encrypted) {
                                        $this->ipcKernelConnection->execAsync('msgDbMsgAdd', [$msg]);
    ...
    private function handleCommandNick($line)
    {
        ...
        if ($data) {
            ...
            $this->ipcKernelConnection->execAsync('setSettingsUserNickname', [$this->userNickname]);
    ...
    public function connect($uri)
    {
        ...
        $connected = $this->ipcKernelConnection->execSync('serverConnectTalkRequest', [$uri]);
    ...
    private function talkResponseSend(TalkRequest $talkRequest)
    {
        ...
        if ($talkRequest->getStatus() == 1) {
            ...
            $this->ipcKernelConnection->execAsync('addressbookContactAdd', [$contact]);
        }

        $this->ipcKernelConnection->execAsync('serverTalkResponseSend',
            [$talkRequest->getClient(), $talkRequest->getRid(), $talkRequest->getStatus(), $userNickname]);
    }
    ...
    private function talkMsgSend($text, $ignore = false)
    {
        ...
        if ($client) {
            $args = [$this->getModeChannelClient(), $rid, $this->userNickname, $text, $ignore];
    ...
    private function talkUserNicknameChangeSend($userNicknameOld, $userNicknameNew)
    {
        $this->ipcKernelConnection->execAsync('serverTalkUserNicknameChangeSend',
            [$this->getModeChannelClient(), $userNicknameOld, $userNicknameNew]);
    }
    ...
    private function talkCloseSend()
    {
        if ($this->getModeChannelClient()) {
            $rid = (string)Uuid::uuid4();
            $args = [$this->getModeChannelClient(), $rid, $this->userNickname];
