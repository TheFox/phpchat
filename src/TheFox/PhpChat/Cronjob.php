...
class Cronjob extends Thread
{
    ...
    private function pingNodes()
    {
        ...
        if ($this->settings->data['node']['bridge']['client']['enabled']) {
            ...
            foreach ($this->table->getNodesClosestBridgeServer(20) as $nodeId => $node) {
                ...
                $this->ipcKernelConnection->execAsync('serverConnectPingOnly', [$node->getUri()]);
            }
        } else {
    ...
    public function msgDbInit()
    {
        ...
        $this->msgDb = $this->ipcKernelConnection->execSync('getMsgDb', [], 10);
        $this->settings = $this->ipcKernelConnection->execSync('getSettings');
        $this->setTable($this->ipcKernelConnection->execSync('getTable'));
        ...
        foreach ($this->msgDb->getUnsentMsgs() as $msgId => $msg) {
            if (
                ...
                if ($onode && $onode->getSslKeyPub()) {
                    ...
                    if ($this->ipcKernelConnection) {
                        $this->ipcKernelConnection->execAsync('msgDbMsgUpdate', [$msg]);
                    }
                } else {
                    if ($this->ipcKernelConnection) {
                        $this->ipcKernelConnection->execAsync('nodesNewDbNodeAddFind', [$node->getIdHexStr()]);
                    }
                }
            }
        }
    ...
    public function msgDbSendAll()
    {
        ...
        foreach ($processedMsgs as $msgId => $msg) {
            ...
            if (
                ...
                if ($this->ipcKernelConnection) {
                    $this->ipcKernelConnection->execAsync('msgDbMsgSetStatusById', [$msg->getId(), 'X']);
                }
                ...
                if ($this->ipcKernelConnection) {
                    $this->ipcKernelConnection->execAsync('msgDbMsgSetStatusById', [$msg->getId(), 'D']);
                }
        ...
        foreach ($nodeIds as $nodeId => $msgs) {
            ...
            if ($msgIds && $this->ipcKernelConnection) {
                $serverConnectArgs = [$node->getUri(), $msgIds];
        ...
        foreach ($updateMsgs as $msgId => $msg) {
            ...
            if ($this->ipcKernelConnection) {
                $this->ipcKernelConnection->execAsync('msgDbMsgIncForwardCyclesById', [$msg['obj']->getId()]);
            }
        }
                        ...
                        if ($nodeObj->getIdHexStr() == '00000000-0000-4000-8000-000000000000') {
                            ...
                            if ((string)$nodeObj->getUri()) {
                                ...
                                if ($noBridge || $isBridgeServer || $isBridgeService) {
                                    $nodes[] = ['type' => 'connect', 'node' => $nodeObj];
                            ...
                            if ((string)$nodeObj->getUri()) {
                                ...
                                $nodes[] = ['type' => 'enclose', 'node' => $nodeObj];
                            } else {
                                #$this->log->debug('    ID, NO URI');
                                $nodes[] = ['type' => 'find', 'node' => $nodeObj];
                            }
                        }
                    }
                    /*else{
    ...
    public function nodesNewEnclose()
    {
        ...
        if ($this->ipcKernelConnection) {
            $this->nodesNewDb = $this->ipcKernelConnection->execSync('getNodesNewDb', [], 10);
        }
        ...
        foreach ($this->nodesNewDb->getNodes() as $nodeId => $node) {
            ...
            if ($node['type'] == 'connect') {
                if ($node['connectAttempts'] >= 10) {
                    ...
                    if ($this->ipcKernelConnection) {
                        $this->ipcKernelConnection->execAsync('nodesNewDbNodeRemove', [$nodeId]);
                    } else {
                    ...
                    if ($settingsBridgeClient) {
                        if ($nodeObj->getBridgeServer()) {
                            ...
                            $this->log->debug('node remove: ' . $nodeId);
                            $nodes[] = ['type' => 'remove', 'node' => $nodeObj];
                ...
                if ($this->table->nodeFind($nodeObj)) {
                    ...
                    $nodes[] = ['type' => 'find', 'node' => $nodeObj];
                    if ($this->ipcKernelConnection) {
                        $this->ipcKernelConnection->execAsync('serverNodeFind', [$node['id']]);
                        $this->ipcKernelConnection->execAsync('nodesNewDbNodeIncFindAttempt', [$nodeId]);
                    } else {
    ...
    private function nodesNewEncloseServerConnect($node, $nodeId)
    {
        ...
        if ($this->ipcKernelConnection) {
            $connected = $this->ipcKernelConnection->execSync('serverConnectPingOnly', [$node->getUri()]);
            if ($connected) {
                $this->log->debug('node remove: ' . $nodeId);
                $this->ipcKernelConnection->execAsync('nodesNewDbNodeRemove', [$nodeId]);
            } else {
                $this->log->debug('node inc connect attempt: ' . $nodeId);
                $this->ipcKernelConnection->execAsync('nodesNewDbNodeIncConnectAttempt', [$nodeId]);
            }
        } else {
