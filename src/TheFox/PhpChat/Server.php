...
class Server
{
    ...
    public function connect($uri, $clientActions = [])
    {
    ...
    public function consoleMsgAdd($msgText = '', $showDate = false, $printPs1 = false, $clearLine = false)
    {
        if ($this->getKernel() && $this->getKernel()->getIpcConsoleConnection()) {
            $this->getKernel()->getIpcConsoleConnection()->execAsync('msgAdd',
                [$msgText, $showDate, $printPs1, $clearLine])
            ;
        }
    }
    ...
    {
        if ($this->getKernel() && $this->getKernel()->getIpcConsoleConnection()) {
            $this->getKernel()->getIpcConsoleConnection()->execAsync('setModeChannel', [$modeChannel]);
        }
    }
    ...
    {
        if ($this->getKernel() && $this->getKernel()->getIpcConsoleConnection()) {
            $this->getKernel()->getIpcConsoleConnection()->execAsync('setModeChannelClient', [$client]);
        }
    }
